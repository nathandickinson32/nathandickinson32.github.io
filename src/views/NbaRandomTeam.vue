...
const comparisonSides = computed(() => comparisonTeamNames.value.map(name => {
  const team = teams.find(item => item[0] === name);
  const roster = rosterByName[name];
  const rank = rankedTeams.value.findIndex(item => item.name === name) + 1;
  return team && roster ? { name, team, roster, rank, topFive: rankedTeams.value[rank - 1]?.topFive } : null;
}));
...
function ensureComparisonTeams() {
  const availableNames = drawnTeams.value.map(item => item.name);
  const current = comparisonTeamNames.value.filter((name, index, list) => availableNames.includes(name) && list.indexOf(name) === index);
  const playerDefaults = state.players.map(p => p.choice || p.picks[0]).filter(Boolean);
  comparisonTeamNames.value = [...current, ...playerDefaults, ...availableNames]
    .filter((name, index, list) => availableNames.includes(name) && list.indexOf(name) === index)
    .slice(0, 2);
  while (comparisonTeamNames.value.length < 2) comparisonTeamNames.value.push('');
}
function setComparisonTeam(side, name) {
  const other = side === 0 ? 1 : 0;
  if (name && comparisonTeamNames.value[other] === name) return;
  comparisonTeamNames.value[side] = name;
}
async function openRatings(name = null) {
  if (name) browseTeam(name);
  if (drawnTeams.value.length) ensureComparisonTeams();
  ...
  <main class="nba-page">
    ...
    <section v-if="ratingsOpen" id="team-ratings" class="ratings-panel" aria-label="NBA 2K26 team and player ratings">
      ...
      <div v-if="drawnTeams.length" class="drawn-roster-browser">
        <div class="drawn-browser-heading"><div><span class="eyebrow">YOUR CURRENT DRAW</span><strong>Compare two teams side by side</strong><small>Choose any two of your {{ drawnTeams.length }} drawn teams</small></div><div class="drawn-arrows"><button aria-label="Previous drawn team roster" @click="browseDrawnStep(-1)">‹</button><button aria-label="Next drawn team roster" @click="browseDrawnStep(1)">›</button></div></div>
        ...
        <div class="comparison-grid" aria-label="Side-by-side drawn team comparison">
          <article v-for="(side, sideIndex) in comparisonSides" :key="sideIndex" class="comparison-card">
            <label>Team {{ sideIndex + 1 }}
              <select :value="comparisonTeamNames[sideIndex]" :aria-label="`Comparison team ${sideIndex + 1}`" @change="setComparisonTeam(sideIndex, $event.target.value)">
                <option value="" disabled>Choose a team</option>
                <optgroup v-for="(p, playerIndex) in state.players" :key="playerIndex" :label="p.name">
                  <option v-for="team in p.picks" :key="team" :value="team" :disabled="comparisonTeamNames[sideIndex === 0 ? 1 : 0] === team">{{ team }}</option>
                </optgroup>
              </select>
            </label>
            <template v-if="side">
              <div class="comparison-summary">
                <div><span class="roster-abbr" :style="{ background: side.team[2] }">{{ side.team[1] }}</span><strong>{{ side.name }}</strong><small>{{ tierLabel(side.name) }} · #{{ side.rank }} rank</small></div>
                <div><strong>{{ side.topFive?.toFixed(1) }}</strong><small>Top 5 avg</small></div>
              </div>
              <div class="comparison-roster"><table><thead><tr><th scope="col">#</th><th scope="col">Player</th><th scope="col">Pos</th><th scope="col">OVR</th></tr></thead><tbody><tr v-for="(p, playerIndex) in side.roster.players" :key="p.name"><td>{{ playerIndex + 1 }}</td><td>{{ p.name }}</td><td>{{ p.position }}</td><td><strong>{{ p.ovr }}</strong></td></tr></tbody></table></div>
            </template>
            <p v-else class="comparison-empty">Choose a team to compare.</p>
          </article>
        </div>
      </div>
...
.comparison-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:15px;padding-top:15px;border-top:1px solid #30445a}.comparison-card{min-width:0;background:#17283c;border:1px solid #40556d;border-radius:11px;overflow:hidden}.comparison-card>label{display:flex;align-items:center;justify-content:space-between;gap:9px;padding:10px 12px;background:#21344b;color:#b9c8d8;font-size:.7rem;font-weight:800;text-transform:uppercase;letter-spacing:.07em}.comparison-card select{min-width:0;max-width:72%;background:#122137;color:#fff;border:1px solid #63788f;border-radius:7px;padding:7px;font:inherit;text-transform:none;letter-spacing:0}.comparison-summary{display:flex;justify-content:space-between;align-items:center;gap:10px;padding:14px 13px;border-bottom:1px solid #354960}.comparison-summary>div:first-child{display:grid;grid-template-columns:auto 1fr;align-items:center;column-gap:8px;min-width:0}.comparison-summary .roster-abbr{grid-row:1 / 3}.comparison-summary strong{font-size:.86rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.comparison-summary small{color:#a9b9ca;font-size:.63rem}.comparison-summary>div:last-child{text-align:right;white-space:nowrap}.comparison-summary>div:last-child strong{display:block;color:#ffbd6a;font-size:1.35rem}.comparison-summary>div:last-child small{display:block}.comparison-roster{max-height:390px;overflow:auto}.comparison-roster table{width:100%;border-collapse:collapse;font-size:.76rem}.comparison-roster th{position:sticky;top:0;background:#17283c;color:#a8bcd1;text-align:left;font-size:.61rem;letter-spacing:.06em;text-transform:uppercase}.comparison-roster th,.comparison-roster td{padding:7px 8px;border-bottom:1px solid #2d4056}.comparison-roster td:first-child{color:#ffbd6a;font-weight:800}.comparison-roster td:last-child strong{display:inline-block;background:#426d65;border-radius:5px;padding:3px 6px}.comparison-empty{color:var(--muted);font-size:.78rem;padding:18px}.comparison-card select:focus-visible{outline:2px solid var(--accent);outline-offset:2px}
...
@media(max-width:550px){.nba-page{margin:0;padding:18px 12px}.page-subtitle{font-size:.82rem}.entry{padding-top:4px}.how-it-works{grid-template-columns:1fr}.entry-buttons{align-items:stretch}.entry-buttons button{flex:1}.join-form>div{display:grid;grid-template-columns:1fr auto}.join-form input{width:100%}.session-progress{gap:4px}.session-progress div{justify-content:center;padding:8px 4px}.session-progress strong{font-size:.62rem}.session-progress span{width:23px;height:23px}.share-controls input{display:none}.share-controls button{flex:1}.tier-filter>span{width:100%}.tier-filter button{flex:1;justify-content:center}.result-card{grid-template-columns:auto 1fr}.result-card button{grid-column:1 / -1;width:100%}.player-head{align-items:flex-start}.pick-player{white-space:normal;max-width:110px}.scoreboard{justify-content:flex-start}.history-heading{align-items:flex-start;flex-direction:column}.ratings-heading-actions{width:100%;justify-content:space-between}.ratings-heading-actions>span{font-size:.72rem}.drawn-roster-browser{padding:12px}.drawn-player-groups{grid-template-columns:1fr}.drawn-player-groups>div>div{display:flex;overflow-x:auto;padding-bottom:5px;scroll-snap-type:x mandatory}.drawn-player-groups button{min-width:210px;scroll-snap-align:start}.comparison-grid{grid-template-columns:1fr}.comparison-roster{max-height:310px}.roster-list-heading{align-items:stretch;flex-direction:column}.roster-list-heading input{width:100%;max-width:none}}
</style>
